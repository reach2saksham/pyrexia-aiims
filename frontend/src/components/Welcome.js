...
const Welcome = () => {
  ...
  useEffect(() => {
    ...
    if (user) {
      axios
        .get(`${BASE_URL}/api/user/status`, { withCredentials: true })
        .then((response) => {
  ...
  const buttonStyle =
    "bg-[#ebe6d0] hover:bg-[#d9d2b8] text-black font-bold py-2 px-4 rounded-lg transition duration-300 text-sm shadow-md";
  const disabledButtonStyle =
    "bg-gray-600 text-gray-300 font-bold py-2 px-4 rounded-lg cursor-not-allowed text-sm";

  const handleTeamChange = (eventId, action) => {
    axios
      .post(
        `${BASE_URL}/api/events/team-size`,
        {
          eventId,
      ...
      .then((response) => {
        setUserStatus((prev) => ({
          ...
          registeredEvents: prev.registeredEvents.map((event) =>
            event._id === eventId
              ? { ...event, teamSize: response.data.teamSize }
              : event
          ),
        }));
      })
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 bg-white/10 backdrop-blur-md p-8 rounded-2xl shadow-lg max-w-3xl w-full mx-4">
        <h1 className="text-3xl lg:text-4xl font-bold mb-2 text-center shackleton-text text-[#ebe6d0]">
          Welcome, {user.name}
        </h1>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-black/20 p-4 rounded-lg">
            <div className="flex justify-between items-center">
              <div>
                ...
                {userStatus.registeredEvents &&
                userStatus.registeredEvents.length > 0 ? (
                    ...
                    {userStatus.registeredEvents.map((event) => (
                      ...
                      >
                        ...
                        <div className="flex space-x-2">
                          ...
                          <button
                            onClick={() =>
                              handleTeamChange(event._id, "decrease")
                            }
                          ...
                          <button
                            onClick={() =>
                              handleTeamChange(event._id, "increase")
                            }
          ...
          <div className="flex justify-between items-center bg-black/20 px-4 py-2 rounded-lg">
            ...
            <Link to="/accomodation" className={buttonStyle}>
              Book Now
            </Link>
          </div>
        </div>
